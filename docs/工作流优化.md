# 工作流优化计划

## 概述

本文档记录 NovelGen 项目后续的工作流优化方向，按优先级排序。

---

## 优化点 1: 动态章节数量（P0 - 高优先级）✅ 已完成

### 当前问题

大纲生成时通过 `settings.num_chapters` 固定章节数量，长篇小说无法根据剧情发展动态调整章节。

### 完成目标

1. 支持分阶段规划：初始只规划开篇章节，后续根据剧情进度动态扩展
2. LLM 可判断「故事是否该结束」，自动触发结局章节生成
3. 保持与现有检查点/断点续跑机制的兼容性

### 实现细节

**方案 A: 分阶段规划（推荐）**

- 修改 `outline_creation_node`，初始只生成「开篇 + 发展」阶段（3-5 章）
- 新增 `evaluate_story_progress_node` 节点，每隔 N 章评估剧情进度
- 新增 `extend_outline_node` 节点，基于当前剧情生成后续章节大纲
- 当评估结果为「可以收尾」时，生成「高潮 + 结局」章节

**状态模型变更**

- `Outline` 添加 `is_complete: bool` 字段标记大纲是否完整
- `Settings` 的 `num_chapters` 改为 `initial_chapters`（初始规划章节数）
- 新增 `max_chapters: int` 字段防止无限生成

**剧情进度评估逻辑**

`evaluate_story_progress_node` 需要综合考虑 `max_chapters` 和当前章节数：

| 条件 | 评估结果 | 后续动作 |
|------|----------|----------|
| 当前章节 >= max_chapters | 强制收尾 | 生成结局大纲（1-2 章） |
| 当前章节 >= max_chapters * 0.8 | 建议收尾 | LLM 评估是否可以自然收尾 |
| 当前章节 < max_chapters * 0.8 | 正常评估 | LLM 自主判断继续/收尾 |

LLM 评估时的 prompt 需包含：
- 剩余可用章节数：`max_chapters - current_chapter`
- 主线剧情完成度（基于大纲中的核心冲突解决进度）
- 未解决的伏笔/支线数量
- 建议：剩余章节不足时，优先收束支线，聚焦主线

**工作流变更**

在 `next_chapter_node` 后添加条件分支：
- 若当前章节 >= 已规划章节数 且 大纲未完成 → `evaluate_story_progress_node`
- 评估结果为「继续」→ `extend_outline_node` → `chapter_planning_node`
- 评估结果为「收尾」→ 生成结局大纲 → 继续章节生成
- 评估结果为「强制收尾」→ 生成压缩版结局大纲（快速收束所有支线）

---

## 优化点 2: 场景生成子工作流（P1 - 中高优先级）✅ 已完成

### 当前问题

`chapter_generation_node` 在单个节点内循环处理所有场景：

```python
# 当前实现（nodes.py 710-836行）
generated_scenes = []
for scene_plan in plan.scenes:
    scene = generate_scene_text(...)
    generated_scenes.append(scene)  # 只在内存中累积
    _save_scene_to_mem0(...)        # 保存到 Mem0，但不保存文件

# 只有全部完成后才保存到文件
chapter = GeneratedChapter(scenes=generated_scenes, ...)
with open(chapter_path, 'w') as f:
    json.dump(chapter.model_dump(), f, ...)
```

**问题**：
1. 逻辑冗长，难以维护
2. 无法实现场景级断点续跑
3. 如果生成第3个场景时失败，前2个场景的文本丢失，需要重新生成

### 完成目标

1. 将场景生成拆分为独立的 LangGraph 子工作流
2. 支持场景级别的检查点，中断后可从具体场景恢复
3. 每个场景完成后立即持久化到文件
4. 为后续「场景级一致性检测」和「场景级修订」做准备

### 实现细节

#### 1. 子工作流结构

```
scene_generation_subgraph
│
├── init_scene_loop          # 初始化：设置 current_scene_number，检测已存在场景
│   └── [条件边] should_generate_scene
│           ├── "skip"    → next_scene_node
│           └── "execute" → retrieve_memory_node
│
├── retrieve_memory_node     # 从 Mem0 检索角色状态和相关记忆
│   └── generate_scene_node
│
├── generate_scene_node      # 调用 LLM 生成场景文本
│   └── save_scene_node
│
├── save_scene_node          # 保存场景到文件和 Mem0
│   └── next_scene_node
│
├── next_scene_node          # 递增 current_scene_number
│   └── [条件边] has_more_scenes
│           ├── "continue" → [条件边] should_generate_scene
│           └── "end"      → END
│
└── END                      # 返回完整的 generated_scenes
```

#### 2. 状态模型设计（关键）

**⚠️ 重要：子图应使用独立的状态类型，不污染父图状态**

根据 LangGraph 最佳实践，父图和子图使用不同的状态类型：

```python
# models.py 新增

class SceneGenerationState(BaseModel):
    """场景生成子工作流状态
    
    独立于 NovelGenerationState，用于场景级别的检查点和断点续跑
    """
    # 章节信息
    chapter_number: int = Field(description="当前章节编号")
    chapter_plan: ChapterPlan = Field(description="章节计划")
    
    # 场景循环控制
    current_scene_number: int = Field(default=1, description="当前场景编号（从1开始）")
    total_scenes: int = Field(default=0, description="总场景数")
    generated_scenes: List[GeneratedScene] = Field(
        default_factory=list, 
        description="已生成的场景列表"
    )
    scene_status: Dict[int, str] = Field(
        default_factory=dict,
        description="场景状态: pending/generating/completed/failed"
    )
    
    # 上下文传递
    previous_summary: str = Field(default="", description="前一场景摘要，用于衔接")
    scene_memory_context: Optional[SceneMemoryContext] = Field(
        default=None, 
        description="当前场景的记忆上下文"
    )
    
    # 从父图传入的只读上下文（必须有默认值以兼容 model_construct）
    world: Optional[WorldSetting] = Field(default=None, description="世界观设定")
    characters: Optional[CharactersConfig] = Field(default=None, description="角色配置")
    project_dir: str = Field(default="", description="项目目录")
    project_name: str = Field(default="", description="项目名称")
    verbose: bool = Field(default=False, description="详细日志")
```

**⚠️ Pydantic 兼容性要求**：

LangGraph 更新状态时可能使用 `model_construct` 绕过验证，因此：
- 所有字段必须有默认值（`default` 或 `default_factory`）
- 可选字段使用 `Optional[T] = None`

#### 3. 检查点管理（关键）

**❌ 错误做法**：子工作流使用独立的 `SqliteSaver`

**✅ 正确做法**：父图和子图共用同一个 checkpointer

根据 LangGraph 文档，检查点会**自动从父图传播到子图**：

```python
# workflow.py

# 父图定义
parent_builder = StateGraph(NovelGenerationState)
parent_builder.add_node("chapter_generation", scene_generation_wrapper_node)
# ... 其他节点

# 子图定义（不设置 checkpointer）
scene_builder = StateGraph(SceneGenerationState)
scene_builder.add_node("init_scene_loop", init_scene_loop_node)
# ... 其他节点
scene_subgraph = scene_builder.compile()  # 不传 checkpointer

# 父图编译时设置 checkpointer，会自动传播到子图
checkpointer = SqliteSaver(conn)
parent_graph = parent_builder.compile(checkpointer=checkpointer)
```

#### 4. 父子图状态转换

需要一个**包装节点**处理 `NovelGenerationState` ↔ `SceneGenerationState` 的转换：

```python
# nodes.py

def scene_generation_wrapper_node(state: NovelGenerationState) -> Dict[str, Any]:
    """
    包装节点：调用场景生成子工作流
    
    职责：
    1. 从父图状态提取子图输入
    2. 检查已存在的场景文件（断点续跑）
    3. 调用子图
    4. 将子图结果合并回父图状态
    """
    chapter_number = state.current_chapter_number
    plan = state.chapters_plan[chapter_number]
    chapters_dir = os.path.join(state.project_dir, "chapters")
    
    # 1. 构建子图输入状态
    subgraph_input = SceneGenerationState(
        chapter_number=chapter_number,
        chapter_plan=plan,
        total_scenes=len(plan.scenes),
        world=state.world,
        characters=state.characters,
        project_dir=state.project_dir,
        project_name=state.project_name,
        verbose=state.verbose
    )
    
    # 2. 检查已存在的场景文件（断点续跑支持）
    for scene_plan in plan.scenes:
        scene_file = os.path.join(
            chapters_dir,
            f"scene_{chapter_number:03d}_{scene_plan.scene_number:03d}.json"
        )
        if os.path.exists(scene_file):
            with open(scene_file, 'r', encoding='utf-8') as f:
                scene = GeneratedScene(**json.load(f))
            subgraph_input.generated_scenes.append(scene)
            subgraph_input.scene_status[scene_plan.scene_number] = "completed"
            print(f"  ⏭️ 场景 {scene_plan.scene_number} 已存在，跳过")
    
    # 设置起始场景编号
    subgraph_input.current_scene_number = len(subgraph_input.generated_scenes) + 1
    
    # 3. 调用子图
    result = scene_generation_subgraph.invoke(subgraph_input)
    
    # 4. 合并场景为章节
    chapter = GeneratedChapter(
        chapter_number=chapter_number,
        chapter_title=plan.chapter_title,
        scenes=result.generated_scenes,
        total_words=sum(s.word_count for s in result.generated_scenes)
    )
    
    # 5. 保存完整章节文件
    chapter_file = os.path.join(chapters_dir, f"chapter_{chapter_number:03d}.json")
    with open(chapter_file, 'w', encoding='utf-8') as f:
        json.dump(chapter.model_dump(), f, ensure_ascii=False, indent=2)
    
    # 6. 清理单独的场景文件（可选）
    # for scene in result.generated_scenes:
    #     scene_file = os.path.join(chapters_dir, f"scene_{chapter_number:03d}_{scene.scene_number:03d}.json")
    #     if os.path.exists(scene_file):
    #         os.remove(scene_file)
    
    chapters = dict(state.chapters)
    chapters[chapter_number] = chapter
    
    return {
        "chapters": chapters,
        "current_step": "chapter_generation",
        "completed_steps": state.completed_steps + [f"chapter_generation_{chapter_number}"]
    }
```

#### 5. 场景持久化策略

**每个场景完成后立即保存到文件**：

```python
# nodes.py - 子图节点

def save_scene_node(state: SceneGenerationState) -> Dict[str, Any]:
    """保存场景到文件和 Mem0"""
    scene = state.generated_scenes[-1]
    chapters_dir = os.path.join(state.project_dir, "chapters")
    os.makedirs(chapters_dir, exist_ok=True)
    
    # 1. 保存场景 JSON 文件
    scene_file = os.path.join(
        chapters_dir,
        f"scene_{state.chapter_number:03d}_{scene.scene_number:03d}.json"
    )
    with open(scene_file, 'w', encoding='utf-8') as f:
        json.dump(scene.model_dump(), f, ensure_ascii=False, indent=2)
    print(f"  💾 场景 {scene.scene_number} 已保存: {scene_file}")
    
    # 2. 保存到 Mem0
    mem0_manager = _get_mem0_manager(state.project_dir, state.project_name)
    if mem0_manager:
        _save_scene_to_mem0(
            mem0_manager=mem0_manager,
            content=scene.content,
            chapter_number=state.chapter_number,
            scene_number=scene.scene_number
        )
    
    # 3. 更新场景状态
    scene_status = dict(state.scene_status)
    scene_status[scene.scene_number] = "completed"
    
    return {"scene_status": scene_status}
```

**文件结构**：

```
projects/demo_001/chapters/
├── scene_001_001.json      # 第1章场景1（中间文件，章节完成后可删除）
├── scene_001_002.json      # 第1章场景2
├── scene_001_003.json      # 第1章场景3
├── chapter_001.json        # 完整第1章（合并所有场景）
├── chapter_001_plan.json   # 第1章计划
└── ...
```

#### 6. 子图节点实现

```python
# nodes.py - 子图节点

def init_scene_loop_node(state: SceneGenerationState) -> Dict[str, Any]:
    """初始化场景循环"""
    print(f"📝 开始生成第 {state.chapter_number} 章的场景...")
    print(f"   总场景数: {state.total_scenes}, 已完成: {len(state.generated_scenes)}")
    
    return {
        "current_scene_number": len(state.generated_scenes) + 1
    }


def should_generate_scene(state: SceneGenerationState) -> Literal["skip", "execute"]:
    """条件函数：判断当前场景是否需要生成"""
    scene_num = state.current_scene_number
    
    if scene_num in state.scene_status and state.scene_status[scene_num] == "completed":
        return "skip"
    return "execute"


def retrieve_memory_node(state: SceneGenerationState) -> Dict[str, Any]:
    """从 Mem0 检索记忆上下文"""
    scene_num = state.current_scene_number
    scene_plan = state.chapter_plan.scenes[scene_num - 1]  # 索引从0开始
    
    mem0_manager = _get_mem0_manager(state.project_dir, state.project_name)
    scene_memory_context = _retrieve_scene_memory_context(
        mem0_manager=mem0_manager,
        scene_plan=scene_plan,
        chapter_number=state.chapter_number,
        project_name=state.project_name
    )
    
    return {"scene_memory_context": scene_memory_context}


def generate_scene_node(state: SceneGenerationState) -> Dict[str, Any]:
    """生成场景文本"""
    scene_num = state.current_scene_number
    scene_plan = state.chapter_plan.scenes[scene_num - 1]
    
    print(f"  ✍️ 生成场景 {scene_num}...")
    
    scene = generate_scene_text(
        scene_plan=scene_plan,
        world_setting=state.world,
        characters=state.characters,
        previous_summary=state.previous_summary,
        chapter_context="",
        scene_memory_context=state.scene_memory_context,
        verbose=state.verbose
    )
    
    # 更新 previous_summary 用于下一场景
    new_summary = scene.content[:200] + "..." if len(scene.content) > 200 else scene.content
    
    return {
        "generated_scenes": state.generated_scenes + [scene],
        "previous_summary": new_summary
    }


def next_scene_node(state: SceneGenerationState) -> Dict[str, Any]:
    """递增场景编号"""
    return {
        "current_scene_number": state.current_scene_number + 1
    }


def has_more_scenes(state: SceneGenerationState) -> Literal["continue", "end"]:
    """条件函数：判断是否还有更多场景"""
    if state.current_scene_number <= state.total_scenes:
        return "continue"
    return "end"
```

#### 7. 子图定义

```python
# workflow.py

def create_scene_generation_subgraph():
    """创建场景生成子工作流"""
    from langgraph.graph import StateGraph, START, END
    from novelgen.models import SceneGenerationState
    
    builder = StateGraph(SceneGenerationState)
    
    # 添加节点
    builder.add_node("init_scene_loop", init_scene_loop_node)
    builder.add_node("retrieve_memory", retrieve_memory_node)
    builder.add_node("generate_scene", generate_scene_node)
    builder.add_node("save_scene", save_scene_node)
    builder.add_node("next_scene", next_scene_node)
    builder.add_node("skip_scene", lambda s: {})  # 空操作
    
    # 定义边
    builder.add_edge(START, "init_scene_loop")
    
    builder.add_conditional_edges(
        "init_scene_loop",
        should_generate_scene,
        {"skip": "skip_scene", "execute": "retrieve_memory"}
    )
    
    builder.add_edge("retrieve_memory", "generate_scene")
    builder.add_edge("generate_scene", "save_scene")
    builder.add_edge("save_scene", "next_scene")
    builder.add_edge("skip_scene", "next_scene")
    
    builder.add_conditional_edges(
        "next_scene",
        has_more_scenes,
        {"continue": "init_scene_loop", "end": END}  # 注意：回到 init 以触发条件判断
    )
    
    # 注意：不传 checkpointer，由父图传播
    return builder.compile()

scene_generation_subgraph = create_scene_generation_subgraph()
```

#### 8. 断点续跑验证

**场景 A：中途中断后恢复**

1. 生成场景1 → 保存 `scene_001_001.json`
2. 生成场景2 → 保存 `scene_001_002.json`
3. 生成场景3 时中断 ❌
4. 恢复运行 → 检测到场景1、2已存在 → 跳过
5. 继续从场景3生成 ✅

**场景 B：完全从头运行**

1. 无场景文件 → `current_scene_number = 1`
2. 依次生成所有场景

---

## 优化点 3: 场景并行生成（P1 - 中高优先级）

### 当前问题

场景按顺序串行生成，每章 4-6 个场景时生成时间较长。

### 完成目标

1. 支持同一章节内多个场景并行生成（配置化开关）
2. 并行生成后自动拼接，确保场景衔接自然
3. 提供场景依赖声明机制，依赖场景串行，独立场景并行

### 实现细节

**前置依赖**：需先完成优化点2（场景生成子工作流）

**并行策略**

- 分析场景计划中的 `characters` 和 `location` 字段
- 若两个场景无共同角色且地点不同，可并行生成
- 否则按顺序串行生成

**LangGraph 实现**

- 使用条件边动态决定并行/串行路径
- 并行分支使用 `fanout` 模式，每个分支生成一个场景
- 汇聚后执行 `merge_scenes_node` 检查衔接一致性

**后处理**

- 新增 `scene_coherence_check_node` 检查并行场景的衔接
- 若发现冲突（如时间线矛盾），触发局部修订
- 配置项 `parallel_scene_generation: bool` 控制是否启用

**风险与限制**

- 并行生成的场景可能存在细节冲突，需要后处理修复
- API 并发调用量增加，需注意 Rate Limit
- 建议初期仅对「独立性高」的场景启用并行

---

## 优化点 4: Function Call 按需检索（P2 - 中优先级）

### 当前问题

场景生成前预先检索所有可能相关的 Mem0 上下文，可能过度检索或遗漏关键信息。

### 完成目标

1. LLM 在生成过程中可自主调用上下文检索工具
2. 减少 prompt 长度，只注入真正需要的信息
3. 为后续 Agent 模式扩展打基础

### 实现细节

**Tool 定义**

定义以下 LangChain Tools 供 LLM 调用：

- `get_character_state(name: str)` - 获取指定角色当前状态
- `search_related_events(query: str)` - 搜索相关剧情事件
- `get_location_info(location: str)` - 获取地点详细信息
- `get_chapter_summary(chapter_num: int)` - 获取指定章节摘要

**调用控制**

- 设置 `max_tool_calls: int = 5` 限制单次生成的最大调用次数
- Tool 返回结果自动注入到后续 prompt 中
- 使用 `create_react_agent` 或 `AgentExecutor` 管理调用循环

**成本考量**

- Function Call 会增加 API 调用次数和 token 消耗
- 建议通过配置项 `use_function_call_retrieval: bool` 控制
- 短篇小说可关闭此功能，长篇小说记忆库大时启用

**实现步骤**

1. 在 `scene_text_chain.py` 中创建 Tool 绑定版本的 chain
2. 修改 `generate_scene_text` 函数支持 Agent 模式
3. 在 `nodes.py` 中根据配置选择调用哪个版本

---

## 优先级总结

| 序号 | 优化点 | 优先级 | 预估工作量 | 依赖关系 | 状态 |
|------|--------|--------|-----------|----------|------|
| 1 | 动态章节数量 | P0 | 3-5 天 | 无 | ✅ 已完成 |
| 2 | 场景生成子工作流 | P1 | 3-4 天 | 无 | ✅ 已完成 |
| 3 | 场景并行生成 | P1 | 2-3 天 | 依赖优化点 2 | 🔲 待开始 |
| 4 | Function Call 按需检索 | P2 | 2-3 天 | 无 | 🔲 待开始 |

---

## 后续扩展方向（待评估）

- **分层记忆架构**：短期/中期/长期记忆分离，优化长篇小说的检索效率
- **场景级一致性检测**：在场景生成后立即检测，而非等整章完成
- **多模型协作**：不同步骤使用不同模型（如 GPT-4 做规划，GPT-3.5 做生成）

---

## 技术要点备忘

### LangGraph 子图集成

1. **检查点传播**：父图的 checkpointer 会自动传播到子图，无需单独配置
2. **状态隔离**：子图使用独立的状态类型，避免污染父图状态
3. **状态转换**：通过包装节点处理父子图状态的转换
4. **Command.PARENT**：子图节点可使用 `Command(graph=Command.PARENT)` 返回父图

### Pydantic 兼容性

1. 所有状态字段必须有默认值（`default` 或 `default_factory`）
2. 可选字段使用 `Optional[T] = None` 而非裸类型
3. LangGraph 可能使用 `model_construct` 跳过验证，避免依赖验证器的副作用
